    ...
    try {
        if (isset($_GET['ordID']) && !empty($_GET['ordID'])) {
            ...
            $sql = "SELECT ordID, ordName, ordPersonal, concat(perName, ' ', perLastName) as ordPersonalName, ordxRef, ordTrnRef,
                ...
                where ordID = :id AND o.ordName = 'Adjustment'
...
function create_order(){
    ...
    try {
        ...
        foreach($records  as $record){
            ...
            $stmt2->execute([$ordID, $proID, $eType, $stgID, round($qty, 3), round($pPrice, 4), round($pPrice, 4)]);
        }

        if($totalProAmount <= $limit){
            $authUser = $usrID;
            $status = 1;
        ...
        $stmt4->execute([$trnRef, $defaultCcy, $branch, $proAccount, "Cr", $totalProAmount, $remark, $entryDateTime]); //Product
        $stmt4->execute([$trnRef, $defaultCcy, $branch, $adjAccount, "Dr", $totalProAmount, $remark, $entryDateTime]); //Expense
        ...
        $value->generateUserActivityLog(
            $user, 
            $oName, 
            "Adjustment Order ID: $ordID,\nTrnRef: $trnRef\nTotal Amount: $totalProAmount,\nxRef: $xRef"
        );
